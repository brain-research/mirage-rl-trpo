...
#  plt.plot(np.mean(np.array(v), axis=0), color=colors[i], label=k)
  for j, res in enumerate(v):
    fields = ['mse_v_lbfgs', 'mse_v', 'mse_q',] # 'mse_model']

    for k, field in enumerate(fields):
      y = [row[field]/row['mse_v_lbfgs'] for row in res]
      if j == 0:
        plt.plot(savgol_filter(y, 21, 5), color=colors[k], label=field)
        plt.plot(y, color=colors[k], alpha=0.1)
      else:
        pass
